...
function tickSpec(start2, stop, count2) {
  const step = (stop - start2) / Math.max(0, count2), power = Math.floor(Math.log10(step)), error = step / Math.pow(10, power), factor2 = error >= e10 ? 10 : error >= e5 ? 5 : error >= e2 ? 2 : 1;
  let i1, i2, inc2;
  if (power < 0) {
    inc2 = Math.pow(10, -power) / factor2;
...
function ticks(start2, stop, count2) {
  ...
  if (start2 === stop)
    return [start2];
  const reverse = stop < start2, [i1, i2, inc2] = reverse ? tickSpec(stop, start2, count2) : tickSpec(start2, stop, count2);
  if (!(i2 >= i1))
    return [];
...
function tickStep(start2, stop, count2) {
  stop = +stop, start2 = +start2, count2 = +count2;
  const reverse = stop < start2, inc2 = reverse ? tickIncrement(stop, start2, count2) : tickIncrement(start2, stop, count2);
